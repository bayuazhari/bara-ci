			...
			<div class="panel panel-inverse">
				...
				<div class="panel-heading">
					...
					<?php if(@$checkLevel->create == 1){ ?>
						<a href="<?php echo base_url('currency/add/'); ?>" class="btn btn-xs btn-circle btn-primary"><i class="fa fa-plus"></i> Add New</a>
						<a href="<?php echo base_url('currency/bulk_upload/'); ?>" class="btn btn-xs btn-circle btn-success"><i class="fa fa-upload"></i> Bulk Upload</a>
					<?php } ?>
									...
									<?php if(@$checkLevel->update == 1 OR @$checkLevel->delete == 1){ ?>
										<div class="btn-group">
											...
											<?php if(@$checkLevel->update == 1){ ?>
												<a href="<?php echo base_url('currency/edit/'.$row->currency_id); ?>" class="dropdown-item"><i class="fa fa-edit"></i> Edit</a>
											<?php } if(@$checkLevel->delete == 1){ ?>
												<a href="javascript:;" class="dropdown-item <?php if(@$model->getCurrencyRelatedTable('country', $row->currency_id)){ echo 'disabled'; } ?>"  data-toggle="modal" data-target="#modal-delete" data-href="<?php echo base_url('currency/delete/'.$row->currency_id) ?>"><i class="fa fa-trash-alt"></i> Delete</a>
											<?php } ?>
											</div>
										</div>
