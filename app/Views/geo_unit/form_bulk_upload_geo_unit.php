		...
		<div id="content" class="content">
			...
			<ul class="timeline">
				<li>
					...
					<div class="timeline-body">
						<div class="timeline-header">
							...
							<span class="views"><a href="<?php echo base_url('geo_unit') ?>" class="btn btn-default btn-sm"><i class="fa fa-arrow-circle-left"></i> Back</a></span>
						</div>
						<div class="timeline-content">
							<a href="<?php echo base_url('assets/templates/geo_units.csv') ?>" class="btn btn-info btn-block"><i class="fa fa-download"></i> Download CSV Template</a>
						</div>
					</div>
					<!-- end timeline-body -->
				</li>
				<li>
					...
					<div class="timeline-body">
						<div class="timeline-header">
							<span class="username">Add geographical unit info in CSV template</span>
						</div>
						<div class="timeline-content">
							<p>
							...
							<p>
								Description:<br>
								<strong>code</strong> - Geographical unit code (e.g., JW).<br>
								<strong>name</strong> - Geographical unit name (e.g., Jawa).
							</p>
						</div>
					</div>
					<!-- end timeline-body -->
				...
				<li>
					...
					<div class="timeline-body">
						...
						<div class="timeline-content">
							<p>
								The upload geographical units file has fields separated by a comma only. The first line contains the valid field names. The rest of the lines (records) contain information about each geographical unit.<br>
						...
						<div class="timeline-comment-box">
							<div class="input">
								<form action="<?php echo base_url('geo_unit/bulk_upload') ?>" method="post" enctype="multipart/form-data">
									<?php $error = $validation->getError('geo_unit_csv'); ?>
									<div class="input-group">
										<input type="file" name="geo_unit_csv" class="form-control rounded-corner <?php if($error){ echo 'is-invalid'; } ?>" />
				...
				<li>
					...
					<div class="timeline-body">
						...
						<div class="timeline-content">
							<form action="<?php echo base_url('geo_unit/bulk_save') ?>" method="post">
							<div class="table-responsive">
								<table class="table table-striped table-bordered table-td-valign-middle">
									...
									<?php
										...
										foreach ($geo_unit as $row) {
											$geo_unit_code = $model->getGeoUnitByField('geo_unit_code', @$row['code']);

											if(empty(@$row['code']) OR (is_string(@$row['code']) != 1) OR (strlen(@$row['code']) != 2) OR @$geo_unit_code){
												$code_error = true;
											}else{
												$code_error = false;
											}
									...
									?>
										<tr>
											<td class="<?php if($code_error == true){ echo 'bg-red text-white'; } ?>"><input type="hidden" name="geo_unit[<?= $no ?>][geo_unit_code]" value="<?= @$row['code'] ?>"><?= @$row['code'] ?></td>
											<td class="<?php if($name_error == true){ echo 'bg-red text-white'; } ?>"><input type="hidden" name="geo_unit[<?= $no ?>][geo_unit_name]" value="<?= @$row['name'] ?>"><?= @$row['name'] ?></td>
										</tr>
									<?php $no++; } ?>
									</tbody>
								</table>
