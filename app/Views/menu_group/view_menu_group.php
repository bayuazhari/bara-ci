									...
									<?php if(@$checkLevel->update == 1 OR @$checkLevel->delete == 1){ ?>
										<div class="btn-group">
											...
											<?php if(@$checkLevel->update == 1){ ?>
												<a href="<?php echo base_url('menu_group/edit/'.$row->mgroup_id); ?>" class="dropdown-item"><i class="fa fa-edit"></i> Edit</a>
											<?php } if(@$checkLevel->delete == 1){ ?>
												<a href="javascript:;" class="dropdown-item <?php if(@$model->getMenuGroupRelatedTable('menu', $row->mgroup_id)){ echo 'disabled'; } ?>"  data-toggle="modal" data-target="#modal-delete" data-href="<?php echo base_url('menu_group/delete/'.$row->mgroup_id) ?>"><i class="fa fa-trash-alt"></i> Delete</a>
											<?php } ?>
											</div>
										</div>
